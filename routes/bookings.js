...
router.put("/:bookingId", async (req, res) => {
    ...
    try {
        ...
        if (result.data.length === 0) {
            ...
            let sql = `
                UPDATE bookings 
                SET booking_description = '${bookingDescription}', proposed_date = '${proposedDate}', estimated_time = ${estimatedTime}, need_donation = ${needDonation}, booking_status = '${bookingStatus}', service_time = ${serviceTime}, fk_requestor_id = ${requestor.userID}, fk_service_post_id = ${servicePost.serviceProvider}
                WHERE id = ${bookingId}
            `;
